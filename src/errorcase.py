...
from visualize import visualize_test
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--num-cases', default=9, help='maximum number of error cases')
        ...
        for j in range(len(compare_config['scheduler'])):
            for k, batch_size in enumerate(compare_config['batch_size']):
                ...
                model: ImageClsMLP = ImageClsMLP(test_set.image_size, 10, [best_hidden[real_name]],
                                                 config['dropout_rate'])
                model.load_params(get_path(root_path, 'model', name) / f'{real_name}.pkl')
                ...
                visualize_test(np.concatenate(error_input), np.concatenate(error_output),
